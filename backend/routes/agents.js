...
const shield = async (req, res, endpointPath) => {
  ...
  try {
    ...
    if (cached) {
      console.log(
        `[Shield] Cache HIT for ${endpointPath} (Hash: ${hash.substring(0, 8)})`,
      );
      return res.json(cached.responseData);
    }

    console.log(
      `[Shield] Cache MISS for ${endpointPath} (Hash: ${hash.substring(0, 8)}). Fetching from Agents...`,
    );
...
// GET /api/agents/analytics/tokens - Token usage analytics (no caching)
router.get("/analytics/tokens", async (req, res) => {
  try {
    const agentsUrl = process.env.AGENTS_API_URL || "http://localhost:8000";
    const response = await axios.get(`${agentsUrl}/analytics/tokens`);
...
// POST /api/agents/analytics/tokens/reset - Reset token analytics
router.post("/analytics/tokens/reset", async (req, res) => {
  try {
    const agentsUrl = process.env.AGENTS_API_URL || "http://localhost:8000";
    const response = await axios.post(`${agentsUrl}/analytics/tokens/reset`);
    res.json(response.data);
  } catch (error) {
    console.error(
      "[Analytics] Error resetting token analytics:",
      error.message,
    );
    if (error.response) {
      return res.status(error.response.status).json(error.response.data);
